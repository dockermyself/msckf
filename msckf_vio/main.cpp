...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        data_dir = std::string("../msckf_vio/data/euroc/MH_01_easy");
    }
    ...
    if (argc < 3)
    {
        config_file = std::string("/msckf_vio/entry/config/camchain-imucam-euroc.yaml");
    }
    else
    {
    ...
    std::thread *pThreadViewer =
        new std::thread(&MsckfSystem::saveResult, pSystem);
    ...
    {
        while (pSystem->isImageLoopBusy() || pSystem->isFusionLoopBusy())
            ;
    ...
    std::vector<Sensor_imu_t> &imu_data)
{
    ...
    while (getline(filestream, line))
    {
        sscanf(line.c_str(), "%lu,%lf,%lf,%lf,%lf,%lf,%lf",
               &imu.stamp,
               &vel[0], &vel[1], &vel[2],
               &acc[0], &acc[1], &acc[2]);

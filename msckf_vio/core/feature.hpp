...
#include "core/imu_state.h"
#include "core/cam_state.h"
...
namespace msckf_vio
{
    ...
    public:
        ...
        struct OptimizationConfig
        {
            ...
            int32_t outer_loop_max_iteration;
            int32_t inner_loop_max_iteration;
     ...
     */
    class Feature : public FeatureStatic<void>
    {
    public:
    ...
    template <typename T>
    double FeatureStatic<T>::observation_noise = 0.01;

    template <typename T>
    typename FeatureStatic<T>::OptimizationConfig
        FeatureStatic<T>::optimization_config;
        ...
        const CamStateServer &cam_states) const
    {
        ...
        first_cam_pose.linear() = quaternionToRotation(
                                      cam_states.find(first_cam_id)->second.orientation)
                                      .transpose();
        ...
        const CamStateServer &cam_states)
    {
        ...
        int32_t inner_loop_cntr = 0;
        int32_t outer_loop_cntr = 0;
        bool is_cost_reduced = false;
        double delta_norm = 0;

        // Compute the initial cost.
        double total_cost = 0.0;
        for (uint32_t i = 0; i < cam_poses.size(); ++i)
        {
        ...
        do
        {
            ...
            outer_loop_cntr++ < optimization_config.outer_loop_max_iteration &&

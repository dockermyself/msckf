...
namespace msckf_vio
{
    ...
    ImageProcessor::ImageProcessor() : is_first_img(true),
                                       next_feature_id(0),
    ...
    void ImageProcessor::initializeFirstFrame()
    {
        ...
        for (int32_t code = 0; code < grid_cnt; ++code)
        {
            ...
            for (int32_t k = 0; k < processor_config.grid_min_feature_num &&
                                k < (int32_t)new_features_this_grid.size();
                 ++k)
            {
        ...
        const std::deque<Sensor_imu_t> &imu_buffer)
    {
        ...
        float dtime = (float)(((double)cam0_curr_img.stamp - (double)cam0_prev_img.stamp) / 1e9);
    ...
    void ImageProcessor::drawFeaturesStereo()
    {
        ...
        for (const auto &id : prev_ids)
        {
                ...
                curr_cam0_points.find(id) != curr_cam0_points.end())
            {
                ...
                cv::Point2f prev_pt1 = prev_cam1_points[id] + cv::Point2f((float)img_width, 0.0f);
                cv::Point2f curr_pt0 = curr_cam0_points[id];
                cv::Point2f curr_pt1 = curr_cam1_points[id] + cv::Point2f((float)img_width, 0.0f);

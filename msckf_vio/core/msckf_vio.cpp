...
namespace msckf_vio
{
    ...
    MsckfVio::MsckfVio() : is_gravity_set(false),
                           is_first_img(true)
    {
    }
        ...
        std::vector<Vector3f_t> &feature3d)
    {
        ...
        Eigen::Vector4d q = rotationToQuaternion(T_b_w.linear());
        odom.orientation.x = (float)q.x();
        odom.orientation.y = (float)q.y();
        odom.orientation.z = (float)q.z();
        odom.orientation.w = (float)q.w();
        ...
        odom.velocity.x = (float)body_velocity(0);
        odom.velocity.y = (float)body_velocity(1);
        odom.velocity.z = (float)body_velocity(2);
        ...
        for (const auto &item : map_server)
        {
            ...
            if (feature.is_initialized)
            {
                ...
                feature3d.push_back(feature_pt);
            }
        }
    }
